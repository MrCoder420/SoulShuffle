...
function run() {
  ...
  // Instead of line by line, use robust regex matching the outer blocks!
  const blockStart = "room && room.status === 'ACTIVE' ? (";
  const blockEnd = ") : (\n        <ScrollView \n          contentContainerStyle={{ flexGrow: 1 }}\n          showsVerticalScrollIndicator={false}\n          refreshControl=";

  const startIndex = content.indexOf(blockStart);
  let endIndex = content.indexOf(blockEnd);

  // If the exact multiline end string isn't found due to different spaces/indents, fallback to regex
  if (endIndex === -1) {
    const endMatch = content.match(/\)\s*:\s*\(\s*<ScrollView\s*contentContainerStyle=\{\{\s*flexGrow:\s*1\s*\}\}\s*showsVerticalScrollIndicator=\{false\}/);
    if (endMatch) {
  ...
  // Slice it exactly!
  const prefix = content.substring(0, startIndex + blockStart.length);
  ...
  const oldHeader = `<View className="flex-row items-center justify-between px-6 pt-5 pb-3 bg-[#fff8f7] dark:bg-[#0F0608] z-10">
  ...
  const newHeader = `<View className="flex-row items-center justify-between px-6 pt-5 pb-3 bg-[#fff8f7] dark:bg-[#0F0608] z-10">
        <TouchableOpacity onPress={openSidebar}>
